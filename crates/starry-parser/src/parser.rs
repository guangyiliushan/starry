...
use crate::analysis::HandleReducer;
use starry_ast::{AstNode, Token, TokenKind, TokenStream};
...
impl Parser {
    ...
    pub fn parse_to_ast(&mut self) -> Result<AstNode, ParseError> {
        let tree = self.parse()?;
        Ok(HandleReducer::new().reduce_all(&tree, &self.cfg))
    }
...
mod tests {
    ...
    #[test]
    fn test_parse_to_ast_with_parens() {
        let grammar_str = r#"
            Expr -> Term + Expr | Term
        ...
        builder.add_operator("+".to_string());
        builder.add_integer(4, "4".to_string());
        builder.add_delimiter(")".to_string());
        let stream = builder.build();
    ...
    #[test]
    fn test_parse_to_ast_uses_exact_production_match() {
        let grammar_str = r#"
            F -> [ E ] | ( E )
            E -> num
        "#;
        ...
        let mut builder = TokenStreamBuilder::new();
        builder.add_delimiter("(".to_string());
        builder.add_integer(7, "7".to_string());
        builder.add_delimiter(")".to_string());
        let stream = builder.build();
