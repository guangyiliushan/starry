...
use crate::lr::error::{LRValidationError, LRValidationReport};
...
impl SLR1Validator {
    ...
    pub fn can_resolve_with_follow(state: &HashSet<LR0Item>, cfg: &ContextFreeGrammar) -> bool {
        ...
        for i in 0..reduce_follows.len() {
            for j in (i + 1)..reduce_follows.len() {
                let intersection: HashSet<_> =
                    reduce_follows[i].intersection(&reduce_follows[j]).collect();
