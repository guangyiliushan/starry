...
impl LRParser {
    pub fn new(cfg: ContextFreeGrammar, table: Box<dyn LRTable>) -> Result<Self, LRError> {
        Self::with_config(cfg, table, LRParserConfig::default())
    }
    ...
    pub fn parse(&self, tokens: &[Token]) -> Result<ParseResult, LRError> {
        ...
        loop {
            let current_state =
                stack.top_state().ok_or_else(LRError::stack_underflow)?;
            let current_token = input
                .get(position)
                .copied()
                .unwrap_or(self.token_mapper.end_marker());
            ...
            match action {
                ...
                Action::Reduce(production_id) => {
                    let production = self
                        .augmented
                        .get_production(production_id.0)
                        .ok_or_else(|| {
                    ...
                    let rhs_len = if production.is_epsilon() {
                        0
                    } else {
                        production.rhs.len()
                    };
                    ...
                    let node = ast_builder.reduce_by_production(production, children);
                    ast_stack.push(node);
...
mod tests {
    ...
    fn test_lr_parser_creation() {
        ...
        let parser = LRParser::new(grammar, Box::new(table));
    ...
    fn test_parse_simple() {
        ...
        let augmented = AugmentedGrammar::new(grammar.clone());
        let table = LR0TableBuilder::build(&augmented);
        let parser = LRParser::new(grammar, Box::new(table)).unwrap();
    ...
    fn test_parse_trace() {
        ...
        let parser = LRParser::with_config(grammar, Box::new(table), config).unwrap();
